...
class XarmReachSacEnv(gym.GoalEnv):
    ...
    def __init__(self, config, render_mode=None):
        ...
        if config['Digital_twin']:
            ...
            self.Cylindermass = 10
            self.CylinderbasePosition = [-0.49, 0.31, 0.06] # intial position of obstacles, later will be updated by ros.
            ...
            self.Cubemass = 1
            self.CubebasePosition = [-0.65, 0.23, 0.01] # intial position of goal, later will be updated by ros.
    ...
    def step(self, action):
        ...
        if self.config['Digital_twin']:
            ...
            # pub_retrain = rospy.Publisher('/Re_taining', Int64, queue_size=10)
        ...
        ###################################################################
        if (self.config['Sim2Real'] and self.num_steps>0):
    ...
    def reset(self):
        super(XarmReachSacEnv, self).reset()
        self.num_steps = 0
        ######################################################
        ...
        if self.config['Digital_twin']:
            ...
            print('Get real obstacle from camera:', self.obstacle)
        else:
            self.goal = self._sample_goal()
            self.obstacle = self._sample_obstacle()
    ...
    def compute_reward(self, action, achieved_goal, observation, goal, info):
        ...
        if (closestDistance_wall_left < 0.1 or closestDistance_wall_right < 0.1):
            Ro_1 = -1
            print('give collision punishment for collision with wall!')
        else:
            Ro_1 = 0

        if ((closestDistance_obs_left < 0.1 or closestDistance_obs_right < 0.1)):
            Ro_2 = -1
    ...
    def _is_success(self, achieved_goal):
        ...
        return ((d < self.distance_threshold).astype(np.float32 ) and self.safe_flag)
